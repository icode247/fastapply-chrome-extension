...
const IndeedJobApplyManager = {
  ...
  async init() {
    ...
    chrome.runtime.onMessageExternal.addListener(this.handleIndeedMessage.bind(this));
  },
  ...
  async handleIndeedMessage(request, sender, sendResponse) {
    ...
    try {
      switch (request.action) {
        ...
        case "startApplying":
          console.log("Started!",request, getJobURL(request.country))
          try {
  ...
  async handleExternalMessage(request, sender, sendResponse) {
    ...
    if (
      ...
      request.url === "https://ng.indeed.com/"
    ) {
      try {
        const tab = await chrome.tabs.create({
          url: getJobURL(request.country),
